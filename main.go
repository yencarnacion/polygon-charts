...
func candlesHandler(w http.ResponseWriter, r *http.Request) {
  ...
  to      := q.Get("to")
  if to == "" { to = q.Get("date") }
  if from == "" { from = q.Get("date") }
  if from == "" || to == "" {
    http.Error(w, "from and to or date required", 400)
    return
  }
  ...
  for _, b := range bars {
    ...
    if !extended && span == "minute" && (h < 9 || (h==9 && m<30) || h >= 16) { continue }
  ...
  if span != "day" {
    ...
    if minErr != nil { http.Error(w, minErr.Error(), 502); return }
  }
